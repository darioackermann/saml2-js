...
describe 'saml2', ->
  ...
  describe 'post_assert', ->
    ...
    it 'errors if passed invalid data', (done) ->
      ...
      sp.post_assert idp, request_options, (err, user) ->
        assert (err instanceof Error), "Did not get expected error."
        done()

    it 'errors if passed invalid XML (repeated attributes)', (done) ->
      sp_options =
        entity_id: 'https://sp.example.com/metadata.xml'
        private_key: get_test_file('test.pem')
        certificate: get_test_file('test.crt')
      ...
      request_options =
        request_body:
          SAMLResponse: get_test_file("post_response_invalid_xml.xml")
      ...
      sp.post_assert idp, request_options, (err, user) ->
        assert (err instanceof Error), "Did not get expected error."
        assert.equal err.message, "Attribute Destination redefined"
        done()
